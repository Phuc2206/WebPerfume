...
function loadData(_page) {
	$.ajax({
		type: 'GET',
		url: '/Admin/Trademarks/GetList?page=' + _page,
...
function render(data) {
	return data.forEach(function (res, index) {
		...
		var td = `		<td>${res.id}</td>
						<td >${res.name}</td>
						<td><img src="/Images/Trademarks/${res.avatar}" class="img-fluid rounded-circle img-responsive" style="height: 50px; width: 50px; right: 10px;"></td>
						<td data-id="${res.id}">
							<div class="btn-group text-center" role="group">
								<a href="#" class="btn btn-sm btn-outline-secondary js-update" data-id=${res.id} data-toggle="modal" data-bs-toggle="modal" data-bs-target="#Trademarks_modal">
...
$("#btnSaveTrademarks").on("click", function (ev) {
	...
	$.ajax({
		url: "/Admin/Trademarks/AddOrUpdate",
		data: data,
		dataType: 'json',
		type: "POST",
		processData: false,
